...
export const F = {
  ...
  copy: ['title', 'describe', 'body', 'adContent', 'content'],
  // "converUrl" is insightrackr's own spelling of cover; there is no coverUrl.
  // imageUrl/thumbnailImageUrl come back as [""] for videos, so firstOf must ignore non-strings.
  thumb: ['converUrl', 'thumbnailConverUrl', 'coverUrl', 'imageUrl', 'picUrl'],
};

const BEIJING_OFFSET = 8 * 3600e3; // epoch dates are Beijing midnight
const num = (v) => (v == null || v === '' ? null : Number(v));
const firstOf = (raw, keys) => keys.map((k) => raw[k]).find((v) => typeof v === 'string' && v !== '');
